defmodule SyphtClient.Result do
  ...
  * `result_error_prefix: "SyphtResult failed:"` - Prefix result error messages with this
      ...
      {:error, status, reason} ->
        {:error,
         SyphtClient.Error.message(
           Application.get_env(:sypht_client, :result_error_prefix),
           status,
           reason
         )}
  ...
  defp http_args(access_token, file_id) do
    %{
      url: "#{Application.get_env(:sypht_client, :result_url)}/#{file_id}",
      options: Application.get_env(:sypht_client, :result_http_options),
